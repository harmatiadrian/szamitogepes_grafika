...
void bounce_ball(Pong* pong)
{
    ...
    if ((pong->ball.x - pong->ball.radius < 50 ) && !((pong->ball.y < (pong->left_pad.y)) || (pong->ball.y > (pong->left_pad.y + pong->left_pad.height)))) 
    {
        ...
        pong->ball.speed_x *= -1;
        printf("ball:%.4f; %.4f pong: %.4f , %.4f\n",pong->ball.x,pong->ball.y,pong->left_pad.y , pong->left_pad.y + pong->left_pad.height);
    }
    ...
    if ((pong->ball.x - pong->ball.radius)==(pong->right_pad.x + pong->right_pad.width))
    {
        pong->ball.x = pong->ball.radius + 50;
        pong->ball.speed_x *= -1;
    }
